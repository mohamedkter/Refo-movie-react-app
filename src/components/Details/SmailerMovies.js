...
const SmailerMovies = (props) => {
const[Movies,setMovies]=useState([]);
const nav=useNavigate();
useEffect(()=>{
    window.scrollTo(0,0)  
  FetchApi(`${props.madiType}/${props.MovieData.id}/similar?language=en-US&page=1`)
  .then(response => setMovies(response.results))
  .catch(err => console.error(err));
},[props.MovieData.id, props.madiType])
  ...
  return (
    <div className='w-full mx-auto mt-20'>
        <div className='w-full  h-[15vh] flex justify-between items-center relative'>
            <h2 className='font-sans text-2xl font-thin'>Similar Movies</h2>
